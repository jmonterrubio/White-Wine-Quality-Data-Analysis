...
```{r echo=FALSE, message=FALSE, warning=FALSE, Sugar_taste}
wines <- mutate(wines,
...
```{r echo=FALSE, message=FALSE, warning=FALSE, Citric_acid_table}
...
```{r echo=FALSE, Density_by_Alcohol_with_Quality}
...
ggplot(aes(x = alcohol, y = quality, colour = log10(residual.sugar)), data = wines) +
  geom_point(position = 'jitter') +
...
ggplot(aes(x = alcohol + log10(residual.sugar), y = quality, colour = total.acidity), data = wines) +
  geom_point(position = 'jitter') +
  coord_cartesian(xlim = c(quantile(wines$alcohol + log10(wines$residual.sugar), .01), quantile(wines$alcohol + log10(wines$residual.sugar), .99))) +
  scale_color_distiller(palette = 'Set1') +
...
ggplot(aes(x = total.acidity, y = quality, colour = alcohol + log10(residual.sugar)), data = wines) +
  geom_point(position = 'jitter') +
...
```{r echo=FALSE, Total_Acidity_vs_Residual_Sugar_and_Alcohol_by_Quality}
ggplot(aes(x = total.acidity, y = alcohol + log10(residual.sugar), colour = quality.factor), data = wines) +
  geom_point(position = 'jitter') +
  coord_cartesian(xlim = c(quantile(wines$total.acidity, .01), quantile(wines$total.acidity, .99))) +
...
plot2 <- ggplot(aes(x = residual.sugar, y = density, colour = quality.factor), data = wines) +
  geom_point(alpha = 0.5, size = 1, position = 'jitter') +
  scale_x_log10(breaks = c(1, 2, 5, 10, 20)) +
  geom_smooth(method = "lm", se = FALSE) +
  facet_wrap(~quality.factor) +
