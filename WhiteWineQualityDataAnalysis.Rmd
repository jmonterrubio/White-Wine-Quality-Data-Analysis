...
```{r echo=FALSE, Density_vs_Alcohol_by_Quality}
...
Here we can see that with higer quality values density vs alcohol values seems to be in the top left of the graph and with lower values density vs alcohol fall in the left side (always following the lm, that look similar for every wine quality).

```{r echo=FALSE, Density_vs_Residual_Sugar_by_Quality}
...
```{r echo=FALSE, Quality_vs_Residual_Sugar_by_Alcohol}
...
```{r echo=FALSE, Quality_vs_Alcohol_by_Residual_Sugar}
ggplot(aes(x = alcohol, y = quality, colour = residual.sugar), data = wines) +
  geom_point(position = 'jitter') +
  coord_cartesian(xlim = c(quantile(wines$alcohol, .01), quantile(wines$alcohol, .99))) +
  scale_color_distiller(palette = 'Set1') +
  ggtitle('Quality vs Alcohol by Residual Sugar')
```

```{r echo=FALSE, Quality_vs_Residual_Sugar_and_Alcohol_by_Total_Acidity}
ggplot(aes(x = alcohol + residual.sugar, y = quality, colour = total.acidity), data = wines) +
  geom_point(position = 'jitter') +
  coord_cartesian(xlim = c(quantile(wines$alcohol + wines$residual.sugar, .01), quantile(wines$alcohol + wines$residual.sugar, .99))) +
  scale_color_distiller(palette = 'Set1') +
  ggtitle('Quality vs Residual Sugar and Alcohol by Total Acidity')
```

```{r echo=FALSE, Quality_vs_Total_Acidity_by_Residual_Sugar_and_Alcohol}
ggplot(aes(x = total.acidity, y = quality, colour = log(alcohol) + log(residual.sugar)), data = wines) +
  geom_point(position = 'jitter') +
  coord_cartesian(xlim = c(quantile(wines$total.acidity, .01), quantile(wines$total.acidity, .99))) +
  scale_color_distiller(palette = 'Set1') +
